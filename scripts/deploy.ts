...
async function deploy() {
  try {
    ...
    if (!exists) {
      throw new Error('Missing ./dist directory, have you run `npm run build && npm run deploy`?');
    }
    ...
    // Support common HA config roots (host and container paths)
    // Prefer host-mapped path first for Docker installs, then container paths
    const directories = ['data/homeassistant', 'config', 'homeassistant'];
    let matched = false;
